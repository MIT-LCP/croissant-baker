...
def write_dicom_slide(path: Path) -> None:
    """A minimal VOLUME whole-slide microscopy instance, with no frames.
    ...
    ds.TotalPixelMatrixColumns = 4096
    ds.TotalPixelMatrixRows = 2048
    ds.ImagedVolumeWidth = 24.5
    ds.ImagedVolumeHeight = 16.4
    ds.ContainerIdentifier = "SLIDE-0001"

    # A slide states its physical scale one nesting down, in the shared
    # functional groups, rather than in the top-level PixelSpacing a single
    # frame image uses.
    measures = Dataset()
    measures.PixelSpacing = [0.00025, 0.00025]
    shared = Dataset()
