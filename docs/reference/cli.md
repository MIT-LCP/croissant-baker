...
* `--genomic-sample-ids`: Emit the sample identifiers a genomic file names in its header (the VCF sample columns). Off by default: for a controlled release those identifiers are a manifest of the cohort, and the counts are emitted either way.
