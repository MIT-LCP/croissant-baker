...
* `--genomic-sample-ids`: Emit the sample identifiers a genomic file names in its header (VCF sample columns, BAM read-group SM tags). Off by default: for a controlled release those identifiers are a manifest of the cohort, and the counts are emitted either way.
